...
const Card: FC<Props> = ({ place }) => {
  return (
    <Link
      to={`/place/${place.id}`}
      className="border border-zinc-200 p-4  gap-5 grid grid-cols-3 min-h-75 bg-white card-hover group overflow-hidden"
    >
      {/* Resim */}
      <div className="overflow-hidden rounded-xl">
        <img
          src={place.image_url}
          alt={place.name}
          className="size-full object-cover transition duration-500 group-hover:scale-110 aspect-square "
        />
      </div>
      {/* İçerik */}
      <div className="col-span-2 flex flex-col justify-between">
        <div className="flex flex-col gap-3 ">
          <div className="flex justify-between items-start gap-2">
            <h1 className="text-lg font-bold text-zinc-800 group-hover:text-blue-600 transition duration-300">
              {place.name}
            </h1>
